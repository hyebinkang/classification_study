...
# from keras import backend as K        딥러닝 모델을 개발하기 위한 고수준 구성요소 제공 라이브러리
####################################################
import os
import cv2
...
image_directory = './cell_images/'                #말라리아 데이터셋
...
for i, image_name in enumerate(
    ...
    if (image_name.split('.')[1] == 'png'):                                 # .이후로 나누기
        image = cv2.imread(image_directory + 'Parasitized/' + image_name)
        image = Image.fromarray(image, 'RGB')       #이미지를 배열로 변환
...
dataset = np.array(dataset)             # array로 만들기
...
X_train, X_test, y_train, y_test = train_test_split(dataset, label, test_size=0.20, random_state=0)         #8:2로 train set 과 test set 나누기
...
model.add(Dense(1))                         #1개의 결과를 내보내야 함
model.add(Activation('sigmoid'))            #이진 분류에서 softmax(다중분류)활용 안 함
