...
image_directory = '../1_ImagePreProcessing/cell_images/Parasitized'
mask_directory = '../1_ImagePreProcessing/cell_images/Uninfected'
...
for i, image_name in enumerate(images):  # Remember enumerate method adds a counter and returns the enumerate object
    if (image_name.split('.')[1] == 'png'):
        ...
        image = cv2.imread(image_directory + image_name, 1)         #RGB로 읽어오기
...
for i, image_name in enumerate(masks):
    if (image_name.split('.')[1] == 'png'):
        image = cv2.imread(mask_directory + image_name, 0)          #gray로 읽어오기
        image = Image.fromarray(image)
        image = image.resize((SIZE, SIZE))
...
# Rename original python file from .py to models.py
