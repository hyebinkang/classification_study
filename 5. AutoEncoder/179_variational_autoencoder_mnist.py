...
x_train = x_train.reshape(x_train.shape[0], img_height, img_width, num_channels)            #60000, 28, 28, 1
...
# REPARAMETERIZATION TRICK , 재매개변수화
# Define sampling function to sample from the distribution, 샘플링할 샘플링 함수를 정의
# Reparameterize sample based on the process defined by Gunderson and Huang into the shape of: mu + sigma squared x eps , 공식 활용
# This is to allow gradient descent to allow for gradient estimation accurately. 그래디언트 추정을 정확하게 수행 할 수 있도록
def sample_z(args):
    z_mu, z_sigma = args
...
# z is the lambda custom layer we are adding for gradient descent calculations using mu and variance (sigma)
...
# Need to start with a shape that can be remapped to original image shape as we want our final output to be same shape original input.,원본 이미지 모양으로 다시 매핑할 수 있도록
# So, add dense layer with dimensions that can be reshaped to desired output shape, dense 레이어를 활용해 reshape
