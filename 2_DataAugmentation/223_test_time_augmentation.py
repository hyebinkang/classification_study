...
p0 = model.predict(np.expand_dims(temp_test_img, axis=0))[0][:, :, 0]                   #prediction 된 이미지 , threshold 이미지가 아니라 prediction이미지

p1 = model.predict(np.expand_dims(np.fliplr(temp_test_img), axis=0))[0][:, :, 0]        #flip left-> right
p1 = np.fliplr(p1)

p2 = model.predict(np.expand_dims(np.flipud(temp_test_img), axis=0))[0][:, :, 0]        #flip up -> down
p2 = np.flipud(p2)
