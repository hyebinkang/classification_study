...
print(max_pool.shape)       #1,2,2,1
print(max_pool)             #7.,3.,6.,9.
...
print(avg_pool.shape)               #1,2,2,1
print(avg_pool)                     #4.25,2.25,4,5.25

# Global average pooling: Average of entire image is calculated instead of a smaller block.  전체이미지의 평균은 더 작은 블록 대신 계산
# Instead of down sampling patches of the input feature map, global pooling down samples the entire feature map to a single value.  전체 피쳐 맵을 단일 값으로 샘플링
# These operations can be useful many way, especially when interested in localized information.
# They are used commonly to replace flattening and dense layers.        flatten, Dense layer 대체 사용
# Models typically end with convolution layers upon which GAP layer will be executed.   GAP: Global Average Pooling
# GAP converts each feature map into a single number.       피처맵을 하나의 숫자로 변환
# Since feature maps learn about objects in the image, the GAP output can be used to map the presence of objects for various classes in images.
# The GAP output is connected to softmax to obtain the multiclass probability distribution.  softmax에 연결되어 다중 클래스 확률분포 얻기
...
image_directory = '../1_ImagePreProcessing/cell_images/'
SIZE = 256
